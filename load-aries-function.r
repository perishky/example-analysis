...
load.aries <- function(path, featureset, time=NULL, detectionp=F) {
    ...
    control.matrix <- read.table(file.path(path, "data", "control_matrix", paste(featureset, "txt", sep=".")),
                                 row.names=1, header=T, sep="\t")
    control.matrix <- control.matrix[colnames(meth),]

    cellcount.filenames <- list.files(file.path(path, "data", "derived", "cellcounts"), ".txt$", full.names=T)
